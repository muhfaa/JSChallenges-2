...
// find purchases made in February
for (let i=0; i<purchases.length; i++) {
    if(purchases[i].created_at.substr(5,2) === '02') {
        console.log('Order with id: ' + purchases[i].order_id + ' was made in February');
    }
}

// find purchases made by Ari
let limit = 300000,
    countAri = 0,
    totalAri = 0,
    totalAnnis = 0,
...
for (let i=0; i<purchases.length; i++) {
    if (purchases[i].customer.name.indexOf('Ari') === 0) {
        countAri++;
        for (let j=0; j<purchases[i].items.length; j++) {
            totalAri += purchases[i].items[j].price;
        }
    }
    if (purchases[i].customer.name.indexOf('Annis') === 0) {
        countAri++;
        for (let j=0; j<purchases[i].items.length; j++) {
            totalAnnis += purchases[i].items[j].price;
        }
    }
    if (purchases[i].customer.name.indexOf('Ririn') === 0) {
        countAri++;
        for (let j=0; j<purchases[i].items.length; j++) {
            totalRirin += purchases[i].items[j].price;
        }
    }
}

// find total purchases lower than 300K
if (totalAri < limit) {
    underLimit.push('Ari');
}
if (totalAnnis < limit) {
    underLimit.push('Annis');
}
if (totalRirin < limit) {
    underLimit.push('Ririn');
}

// log to the console
console.log('Total purchases made by Ari: ' + countAri + ', and the grand total is: ' + totalAri);
